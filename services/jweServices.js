...
const encrypt = async (key, payload) => {
  try {
    ...
    const dataToEncrypt = JSON.stringify(JSON.stringify(payload));
    ...
    const token = await jose.JWE.createEncrypt(encryptOptions, publicKey)
      .update(dataToEncrypt, textEncode)
      .final();
    return token;
  } 
  catch (error) {
...
const decrypt = async (key, token) => {
  try {
    ...
    return JSON.parse(JSON.parse(bufferOriginal.toString(textEncode)));
  } 
  catch (error) {
    return error;
  }
